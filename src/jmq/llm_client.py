...
ZONE_CLASSIFIER_SYSTEM_PROMPT = (
    ...
    "Players very often abbreviate a zone as an acronym formed from its logical words, so check for "
    "this pattern before giving up:\n"
    "- For multi-word zone names, the first letter of each word (e.g. 'tf' for 'Toxxulia Forest', "
    "'lm' for 'Lavastorm Mountains').\n"
    "- For single-word zone names that are really a compound of two logical words, the first letter "
    "of each of those inner words (e.g. 'sf' for 'Skyfire' [Sky+Fire], 'ls' for 'Lavastorm' "
    "[Lava+Storm]).\n"
